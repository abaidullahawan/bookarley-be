...
module Products
  ...
  def favourite_products_for_user(data)
    @data.map { |data|
      (data.active_images.attached? && data.cover_photo.attached?) ? data.as_json({no: 'no assocaited object'}).merge(
        active_images_path: data.active_images.map { |img| url_for(img) }).as_json.merge(
          cover_photo_path: url_for(
            data.cover_photo)) : data.active_images.attached? ? data.as_json({no: 'no assocaited object'}).merge(
              active_images_path: data.active_images.map { |img| url_for(
                img) }) : data.cover_photo.attached? ? data.as_json({no: 'no assocaited object'}).merge(
                  cover_photo_path: url_for(data.cover_photo)) : data.as_json({no: 'no assocaited object'})
    }
  end
